...
namespace EBM2x.WPF
{
    public class WindowsSerialPrinter : IBlueToothService
    {
        ...
        public void PrintJournal(string port, int baudRate, Models.journal.JournalModel journal, bool isReprint)
        {
            ...
            foreach (JournalString node in journal.JournalStringList)
            {
                ...
                else if (node.Type.Equals("qrcode"))
                {
                    ...
                    PrintQrCode(printer, RraSdcService.RECEIPT_URL + "/common/link/ebm/receipt/indexEbmReceiptData?Data=" + node.Data);
        ...
        private void JournalPrintPageUSBII(object sender, PrintPageEventArgs ev)
        {
            ...
            {
                // logo 출력
                using (var stream = ResourceUtil.GetImageStream("EBM2x-200.png"))
                {
                    Image image = Image.FromStream(stream);
                    ev.Graphics.DrawImage(image, (cellWidth * 1) + 3, (cellHeight * (1 + hIndex)) + 3);
                }
                using (var stream = ResourceUtil.GetImageStream("rw-image-200.png"))
                {
                    Image image = Image.FromStream(stream);
                    ev.Graphics.DrawImage(image, (cellWidth * 10) + 3, (cellHeight * (1 + hIndex)));
                }
                hIndex += 4;
            }
            ...
            for (int i = 0; i < journal1.JournalStringList.Count; i++, hIndex++)
            {
                ...
                else if (journal1.JournalStringList[i].Type.Equals("qrcode"))
                {
                    ...
                    string grData = RraSdcService.RECEIPT_URL + "/common/link/ebm/receipt/indexEbmReceiptData?Data=" + journal1.JournalStringList[i].Data;
                    //END
                    Rectangle rectangle;
        ...
        private void InvoicePrintPage(object sender, PrintPageEventArgs ev)
        {
            ...
            for (int i = 0; i < journal3.JournalStringList.Count; i++)
            {
                if (i == 0)
                {
                    ev.Graphics.DrawString(journal3.JournalStringList[i].Data, titleFont, brush, (cellWidth * 24) + 3, (cellHeight * (7 + i)) + 3);
                }
                else
                {
                    ev.Graphics.DrawString(journal3.JournalStringList[i].Data, printFont, brush, (cellWidth * 24) + 3, (cellHeight * (7 + i)) + 1);
                }
            }
            ...
            //qr codes generate
            string data = null;
            for (int i = 0; i < journal5.JournalStringList.Count; i++)
            {
                if (journal5.JournalStringList.Count > 5)
                {
                    if (i == 41)
                    {
                        data = journal5.JournalStringList[i].Data;
                    }
                }
                else
                {
            ...
            string grData = RraSdcService.RECEIPT_URL + "/common/link/ebm/receipt/indexEbmReceiptData?Data=" + data;
            Rectangle rectangle;
            rectangle = new Rectangle((cellWidth * 14) + 56, (cellHeight * 47), (120), (120));
            Image image = GetQrcodeImage(grData);
            ev.Graphics.DrawImage(image, rectangle);



            //string grData = RraSdcService.RECEIPT_URL + "/common/link/ebm/receipt/indexEbmReceiptData?Data=";
            //END
            ...
            //{
            //  ev.Graphics.DrawString("REFUND IS APPROVED ONLY FOR", printFont, brush, (cellWidth * 1) + 3, (cellHeight * 8) + 3);
            //ev.Graphics.DrawString("ORIGINAL SALES RECEIPT", printFont, brush, (cellWidth * 1) + 3, (cellHeight * 9) + 3);
            //sign = -1;
            //}
            //else
            //{
            ...
            for (int i = 0; i < 38; i++)
            {
                ...
                /** Commented By Bright 1.5.2022 While Incorporating Aimee Code
                 * 
                ...
                */
                ev.Graphics.DrawString(itemNode.Qty.ToString("#,##0.####"), itemFont, brush, (cellWidth * 18) + 3, (cellHeight * 14) + 10 + (i * addLineHeight));
                ev.Graphics.DrawString(itemNode.TaxTyCd, itemFont, brush, (cellWidth * 21) + 3, (cellHeight * 14) + 10 + (i * addLineHeight));
                buffer = string.Format("{0:##,##0.####}", itemNode.Prc);
                ev.Graphics.DrawString(buffer, itemFont, brush, (cellWidth * 24) + 3, (cellHeight * 14) + 10 + (i * addLineHeight));
                buffer = string.Format("{0:##,##0.####}", itemNode.TotAmt);
                ev.Graphics.DrawString(buffer, itemFont, brush, (cellWidth * 31) + 3, (cellHeight * 14) + 10 + (i * addLineHeight));
        ...
        private void InvoicePrintPageBase(PrintPageEventArgs ev)
        {
            ...
            using (var stream = ResourceUtil.GetImageStream("rw-image-150.png"))
            {
                Image image = Image.FromStream(stream);
                ev.Graphics.DrawImage(image, (cellWidth * 33) + 3, (cellHeight));
            }

            rectangle = new Rectangle((cellWidth * 1), (cellHeight * 8), (cellWidth * 15), (cellHeight * 3));
            ev.Graphics.DrawRectangle(blackPen, rectangle);

            rectangle = new Rectangle((cellWidth * 24), (cellHeight * 7), (cellWidth * 15), (cellHeight * 4));
            ev.Graphics.DrawRectangle(blackPen, rectangle);
