...
{
    public class CreateSalesExcelUtil
    {
        public void CreateSpreadsheetWorkbook(Stream stream, List<TrnsSaleRecord> listTrnsSaleRecord)
        {
            string jsonRequest = JsonConvert.SerializeObject(listTrnsSaleRecord);
            ...
            if (jArray.Count > 0)
            {
                ...
                CreateSpreadsheetWorkbook(stream, keys, jsonRequest, listTrnsSaleRecord);
            }
        }
        public void CreateSpreadsheetWorkbook(Stream stream, IList<string> titles, string jsonRequest, List<TrnsSaleRecord> listTrnsSaleRecord)
        {
            ...
            if (jArray.Count > 0)
            {
                for (int i = 0; i < jArray.Count; i++)
                {
                    ...
                    // Item 
                    TrnsSaleRecord record = listTrnsSaleRecord[i];
                    List <TrnsSaleItemRecord> listTrnsSaleItemRecord = masterItems.getTrnsSaleItemTable(record.Tin, record.BhfId, record.InvcNo, record.RcptTyCd);
                    string jsonRequestItem = JsonConvert.SerializeObject(listTrnsSaleItemRecord);
