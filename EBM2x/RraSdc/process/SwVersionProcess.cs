...
{
    public class SwVersionProcess
    {
        public async void SwVersionDownloadProcess()
        {
            ...
            {              
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(jsonResponse))
                    {
                        ...
                        {
                            RraSdcReceiveJsonWriter.WriteTransactionSCC("selectSwVersion", jsonResponse);
                            SwVersionResData swVersionResData = swVersionRes.data;

                            if (!swVersionResData.ver.Equals("version"))
                            {
                        ...
                        else if (swVersionRes.resultCd.Equals("001"))
                        {
                        }
                        else
                        {
                            RraSdcReceiveJsonWriter.WriteTransactionERR("selectSwVersion", jsonResponse);
                        }
                    }
                }
                catch (Exception ex)
