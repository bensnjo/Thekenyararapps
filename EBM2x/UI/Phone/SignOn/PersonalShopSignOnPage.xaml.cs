...
namespace EBM2x.UI.Phone.SignOn
{
    [XamlCompilation(XamlCompilationOptions.Compile)]
    public partial class PersonalShopSignOnPage : ContentPage
    {
        bool pageIsActive;
        SignOnNode signOnNode = null;
        StateNodeList stateNodeList = null;

        public PersonalShopSignOnPage()
        {
            SetValue(NavigationPage.HasNavigationBarProperty, false);
            InitializeComponent();

            fixedGrid00.InitializeComponent();
            functionPanel.InitializeComponent();
        ...
        async void OnFunctionCalled(object sender, EventArgs e)
        {
            ...
            switch (((ExtEventArgs)e).FunctionID)
            {
                ...
                case "SignOn":
                case "Enter":
                    // Navigation.InsertPageBefore(new PersonalShopSalesPage(), this);
                    if (stateNodeList.CurrentLineNumber == 1)
                    {
                        ...
                        {
                            EBM2x.UI.UiUtils.MsgBox.DisplayAlert(this, UIManager.Instance().InformationModel.AlertTitle, UIManager.Instance().InformationModel.AlertMessage, "Ok");
                            UIManager.Instance().InputModel.Clear();
                        }
                    }
                       ...
                       if (StateModel.IsIt_OP_NEXT(result))
                        {
                            ...
                            Navigation.InsertPageBefore(new PersonalShopMenuPage(), this);
                            await Navigation.PopAsync();
                        }
                        else if (StateModel.IsIt_OP_ALERT_YES_NO(result))
                        {
                    ...
                    else if (stateNodeList.CurrentLineNumber == 2)
                    {
                        if (signOnNode.CashierID == "99999")
                        {
                            ...
                            if (StateModel.IsIt_OP_NEXT(result))
                            {
                                ...
                                Navigation.InsertPageBefore(new PersonalShopAdminMenuPage(), this);
                                await Navigation.PopAsync();
                            }
                            else if (StateModel.IsIt_OP_ALERT_YES_NO(result))
                            {
                ...
                case "Admin":
                    ...
                    {
                        UIManager.Instance().InputModel.Clear();
                        await Navigation.PushAsync(new PersonalShopCommMenuPage());
                    }
                    break;
                case "RraSdcSend":
                    // 2021.06.09
...
/*
                    ...
                    if (UIManager.Instance().InputModel.EnteredText.Length == 8)
                    {
                        ...
                        if (startDate.Substring(0, 4).Equals("2020"))
                        {
