...
namespace EBM2x.UI.Tablet.Menu
{
    ...
    public partial class CashierShiftPage : ContentPage
    {
        ReadyMoneyList readyMoneyList = null;

        public CashierShiftPage()
        {
            ...
            readyMoneyList = ReadyMoneyProcess.LoadCashierShift();
        ...
        async void OnFunctionCalled(object sender, EventArgs e)
        {
            ...
            switch (((ExtEventArgs)e).FunctionID)
            {
                ...
                case "Confirm":
                    // 저장 및 영수증 출력       
                    ReadyMoneyProcess.SaveCashierShift(readyMoneyList);
                    CashierShiftEndOfTransaction.excuteProcess(readyMoneyList, UIManager.Instance().PosModel, UIManager.Instance().InputModel, UIManager.Instance().InformationModel);
                    UIManager.Instance().InputModel.Clear(); // JINIT_Input Clear 추가
                    Navigation.InsertPageBefore(new CashierShiftPageMainPage(), this);
                    await Navigation.PopAsync();
                    break;
