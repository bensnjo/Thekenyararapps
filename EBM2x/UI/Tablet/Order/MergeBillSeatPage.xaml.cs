...
namespace EBM2x.UI.Tablet.Order
{
    ...
    public partial class MergeBillSeatPage : ContentPage
    {
        DiningTableNode selectOwner;

        bool pageIsActive;

        public MergeBillSeatPage()
        {
        ...
        protected override void OnAppearing()
        {
            ...
            UIManager.Instance().PosModel.SetSalesTitleText("Merge Bill");
                ...
                case "DiningTable":
                    ...
                    else
                    {
                        if (diningTableNode.IsOrdered)
                        {
                            ...
                            {
                                string locationTitle2 = UILocation.Instance().GetLocationText("Merge?");
                                ...
                                if (result)
                                {
                                    DiningTableOrderProcess.MergeOrder(selectOwner.DiningTableCode, diningTableNode.DiningTableCode);

                                    if (DateTime.Compare(diningTableNode.FirstOrderTime, selectOwner.FirstOrderTime) > 0)
                                    {
                                        diningTableNode.FirstOrderTime = selectOwner.FirstOrderTime;
                                    }
                                    diningTableNode.Amount = diningTableNode.Amount + selectOwner.Amount;
