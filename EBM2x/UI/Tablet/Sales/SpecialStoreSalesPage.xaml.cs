...
namespace EBM2x.UI.Tablet.Sales
{
    ...
    public partial class SpecialStoreSalesPage : ContentPage
    {
        bool IsEnteredItem = false;

        public SpecialStoreSalesPage()
        {
        ...
        protected override void OnAppearing()
        {
            ...
            {
                Navigation.InsertPageBefore(new SpecialStoreRefundSalesPage(), this);
                Navigation.PopAsync();
            }
        ...
        async void OnScanTapped()
        {
            ...
            scanPage.OnScanResult += (result) => {
                ...
                Device.BeginInvokeOnMainThread(() => {
                    ...
                    if (StateModel.IsIt_OP_NEXT(resultPlu))
                    {
                        bool stockCheck4 = ChangeQuantityProcess.checkStockProcess(UIManager.Instance().PosModel, 0);
                        if (!stockCheck4)
                        {
