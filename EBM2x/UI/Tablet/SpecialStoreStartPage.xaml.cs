...
namespace EBM2x.UI.Tablet
{
    ...
    public partial class SpecialStoreStartPage : ContentPage
    {
        public SpecialStoreStartPage()
        {
        ...
        async void excuteProcess()
        {
            ...
            else if (StateModel.IsIt_OP_NEXT(result))
            {
                ...
                {
                    string locationTitle = UILocation.Instance().GetLocationText("Warning");
                    string locationMessage = UILocation.Instance().GetLocationText("The previous date is not close.Should we go to the menu?");
                    var retClose = await DisplayAlert(locationTitle, locationMessage, "Yes", "No");
                    if (retClose)
                    {
            ...
            {
                Navigation.InsertPageBefore(new SpecialStoreOpenPage(), this);
                await Navigation.PopAsync();
            }
